...
const Chat = () => {
  ...
  function onSearch(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault()
    if (search.length > 0) {
      const filteredTrainer = trainerList.filter((trainer) => trainer.trainerName.toLowerCase().includes(search.toLowerCase()))
  ...
  function getCurrentPartner() {
    const currentPartner = trainerList.find((trainer) => trainer.conversationID === slug)
              ...
              <div className='space-y-1 pb-24 overflow-auto h-full scrollbar-thumb-blue scrollbar-thumb-rounded scrollbar-track-blue-lighter scrollbar-w-2 scrolling-touch'>
                {
                  trainerList.map((trainer, index) => (
                    <TrainerCard chatId={slug?.toString()} key={index} Trainer={trainer} />
                  ))
                }
              </div>
            </div>
                ...
                <div className='relative flex items-center space-x-4'>
                  <div className='relative'>
                    <div className='relative w-8 sm:w-12 h-8 sm:h-12'>
                      <div>
                        {
                          currentPartner?.userImage ? <Image src={currentPartner.userImage} alt='sas' /> : <FaCircleUser size={50} />
                        }
                      </div>
                    </div>
                  </div>
                  ...
                  <div className='flex flex-col leading-tight'>
                    <div className='text-xl flex items-center'>
                      <span className='text-gray-700 mr-3 font-semibold'>
                        {currentPartner?.trainerName}
                      </span>
                    </div>

                    <span className='text-sm text-gray-600'>
                      {currentPartner?.trainerEmail}
                    </span>
                  </div>
                </div>
              </div>
